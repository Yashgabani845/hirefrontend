...
const AssessmentResultDetail = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const response = await fetch(`https://hirebackend-1.onrender.com/api/results/${assessmentId}`);
  ...
  const handleHire = async (applicantId, jobId) => {
    ...
    try {
      const response = await fetch('https://hirebackend-1.onrender.com/api/hire', {
        method: 'POST',
        headers: {
  ...
  const handleReject = async (applicantId, jobId) => {
    try {
      const response = await fetch('https://hirebackend-1.onrender.com/api/reject', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
